...
Simplified version: Samples 1000 files per schema type and trains 3 compressors.
...
def sample_files_for_schema(folders: list[Path], num_files: int = 10000, seed: int = 42) -> list[Path]:
    """
    ...
    print(f"  Sampled {len(sampled)} files from {len(all_payloads)} total payloads across {len(folders)} folders")

    return sampled


def prepare_schema_train_dir(data_dir: Path, schema_name: str, sampled_files: list[Path]) -> Path:
    """
...
def train_compressor(train_dir: Path, output_path: Path, mode: str, script_dir: Path) -> str:
    """
    ...
    """
    # Construct command
    protobuf_cli = script_dir.parent / "build-install" / "tools" / "protobuf" / "protobuf_cli"
...
def main():
    ...
    print("Sampling 1000 files per schema type for training")
    print("=" * 80)
    print()

    # Discover folders grouped by schema
    print("Discovering data folders...")
    schema_folders = discover_data_folders(data_dir)
    ...
    for schema_name in ["otap", "otlp_metrics", "otlp_traces"]:
        ...
        try:
            ...
            sampled_files = sample_files_for_schema(folders, num_files=1000)
    ...
    print("  data/otap/")
    print("    ├── payload_0000.bin ... payload_0009.bin (1000 training files)")
    print("    └── trained.zlc")
